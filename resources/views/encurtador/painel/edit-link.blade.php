...
@section("conteudo")
    <div class="conteiner">
        <form class="row g-3" id="mainForm" action="{{ route('links.update', $link->id) }}" method="POST">
            @csrf
            @method('PUT')
           
            <h4>Editar Link</h4>
            <div class="col-12">
                <input type="text" class="form-control" id="inputAddress" placeholder="descrição" value="{{ $link->description }}" name="description">
            </div>
    
            <div class="col-12">
                <input type="text" class="form-control" id="inputAddress" placeholder="url" value="{{ $link->link }}" name="link">
            </div>
                ...
                <ul>
                   ...
                   @foreach ($link->locks as $lock)
                        <li>
                            <div class="col-12 blocks" >
                                <select id="inputState" class="form-select" name="categories[]">
                                    @foreach ($categorias as $categoria)
                                        @if ($categoria->id == $lock->category->id)
                                ...
                                @if (!$loop->first)
                                    <a type="button" class="btn btn-danger"  href="{{ route('locks.destroy', $lock->id) }}">
                            ...
                            <div class="col-12">
                                <input type="text" class="form-control" id="inputLink" placeholder="url" name="locks[]" value="{{ $lock->linkLock }}">
                            </div>
                        </li>
                    @endforeach
    ...
    <script>
        function addForm() {
            ...
            newField.innerHTML = `
                ...
                <div class="col-12">
                    <input type="text" class="form-control" placeholder="url" name="locks[]">
                </div>
